...
class CommandRequest( BaseRequest ):
  ...
  def _HandleFixitResponse( self ):
    if not len( self._response[ 'fixits' ] ):
      ...
      # When there are multiple fixit suggestions, present them as a list to the
      # user hand have her choose which one to apply.
      fixit_index = 0
      if len( self._response[ 'fixits' ] ) > 1:
        fixit_index = vimsupport.SelectFromList(
          "Multiple FixIt options are available at this location. Which one "
          "would you like to apply?",
          [ fixit[ 'text' ] for fixit in self._response[ 'fixits' ] ] )

      if fixit_index >= 0:
        chunks = self._response[ 'fixits' ][ fixit_index ][ 'chunks' ]
        try:
